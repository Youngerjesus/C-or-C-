...
/*
모자의 종류가 index로 나타낼 수 있다. 
1. 모자가 겹치면 안된다.
2. arr내가 입은 모자를 넣어주면되잖아. 그리고 그때마다 재귀호출
3.  5번을 입으면 arr[0] =5 
    100번을 입으면 arr[0] =100 
    1번을 입으면 arr[0] =1 
    
    사람이달라져야라니까 
    person[i] -> (사람이 가진 모자) i를 ++해서 매번넘겨준다.
    arr이 독립적으로 분기해야한다 .  DP에서 

    if(arrLengh == person.legnth) return 

    for(j =0 j < caps.length) {
        for(arrlength ){
            if(arr[k] == person[i][j])
            break;
        }
